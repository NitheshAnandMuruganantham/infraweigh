...
export class BillService {
  ...
  async create(
    ...
    try {
      const { secret: private_key } = await this.prisma.weighbridge.findUnique({
        where: {
          id: claims['x-hasura-weighbridge-id'],
        },
        select: {
          secret: true,
        },
      });
      const weight: any = jwt.verify(body.scale_weight, private_key);
      if (weight) {
        const id = uuid();
          ...
          this.prisma.bill.create({
            data: {
              ...
              reference_bill_id: !!body.tare_weight
                ...
                : null,
              scale_weight: parseInt(`${weight.weight}`, 10),
